...
MiniLogoURL = 'https://cytube.xyz/up/src/up9973.png';
...
//AnnouncementHTML	= '<div><font size="4"><a href="https://forms.gle/CHJZ4soDhrgNoTJ26" target="_blank" rel="noreferrer noopener">CTV☆人気投票開催中！</a>※投票期間：9/21月曜日頃まで 投票期間中は投票内容を修正出来ます</font></div>';
AnnouncementHTML = '';
...
AnswersArray = ['そうだね', '駄目です', 'あのさぁ…イワナ、書かなかった？', 'お前、adminを便利屋かなんかと勘違いしちゃいないか？', '幻想郷は全てを受け入れるのよ。それはそれは残酷な話ですわ。',
	...
	'信じるものは救われる', '少なくとも次はないわ。もう一度言うわ、次はないわよ', '恋なんじゃないかな、村紗心の恋、フッ', 'ひったくりに使う馬はユニコーン', '当然很自然吗？', '盗みは良くないぞ　人を困らせるな',
...
$(function () {
	Analects = [];　
	var soundAnalects, soundURL;
	var path1 = "https://dl.dropbox.com/s/9ds1iau94456nea/analects.csv?dl=0";
	var path2 = "https://dl.dropbox.com/s/lizm30jqkwqd6j7/soundURL.csv?dl=0";
	var d = $.Deferred();
	$.when(
		$.ajax({
			url: path1
		...
		for (i = 0; i < soundAnalects.length; i++) {
			SoundFiltersArray[soundAnalects[i]] = soundURL[i];
		}
	});
}());
...
//!koreコマンド
var korearray=[
	'https://youtube.com/watch?v=n_blCr2q9Ig',//yuuhei6
	'https://youtube.com/watch?v=JOOcFAQdCWI',//yuuhei8
	'https://youtube.com/watch?v=hLm7eucX7k8',//yuuhei8.5
	'https://twitch.tv/videos/363763453',//yuuhei13
	'https://youtube.com/watch?v=mQMXUl8n3Cs',//kyuuhei
	'https://youtube.com/watch?v=LrNvBIqlkIM',//genpei
	'https://youtube.com/watch?v=OsJ_6Z28G6g',//nanahei
	'https://youtube.com/watch?v=ioPtkZxUfag',//mikodairaIOSYS
	'https://youtube.com/watch?v=xiaoNJvv0Hg',//mikodairaSonota
	'https://youtube.com/watch?v=QQU9MJ2wIaI',//morihei1
	'https://youtube.com/watch?v=czeLs7a-RmI',//morihei2
	'https://cytube.xyz/r/MIMI-channel',//MIMI
	'https://cytube.xyz/r/tokinoboukensya',//tokibou
	'https://cytube.xyz/r/kuronekoamane',//KRNKAMN
	'http://www.nicovideo.jp/watch/sm27873969',//peropero
	'http://www.nicovideo.jp/watch/sm33245442',//antiman
	'https://youtube.com/watch?v=M2z02w0fE5g',//yamikikaku
	'https://youtube.com/watch?v=UlodMZ3qxRI',//birthishi
	'https://youtube.com/watch?v=mP7FNVZGgCs',//birthhana
	'https://youtube.com/watch?v=0giMmMTBe2A',//hanasupa
];
...
_SHOWELEMENTS = '{"header":1,"logo":1,"motd":1,"announcements":1,"mainheader":1,"playerlabels":1,"chatlabels":1,' +
	...
	'"mediadbbtns":1,"chatcontrols":1,"pollemotebtns":1,"colorsbtn":1,"commondsbtn":1,"chatbtns":1,' +
...
//CTV独自拡張
READCHAT = getOrDefault('SP_readchat', false);
READCHATVOLUME = getOrDefault('SP_readchatvolume', 1.0);
MICINPUT = getOrDefault('SP_micinput', false);
MICINPUT_ADDMODE = getOrDefault('SP_micinput_addmode', true);
SNOWFALL = getOrDefault('SP_snowfall', false);
SHOWPLAYLIST = getOrDefault('SP_showplaylist', false);

//ロード時の動画リスト表示の有無について、前に部屋を出た時のものを適用
//仕舞われるのをできるだけ早くしたほうが軽くなりそうなので上の方に置いてます
(function () {
	var button=document.getElementById("showplaylist");
	if (button != null || SHOWPLAYLIST^button.classList.contains("active")) {
		button.click();
	}
	button.addEventListener("click",function () {
		setOpt('SP_showplaylist',SHOWPLAYLIST=button.classList.contains("active"))
	})
})();
...
CYTUBE = 'https://cytube.xyz/up/src/';
VERSION = '2.12.2';
...
CommondsArray = {
	...
	'an': '<span style="font-size:11px;color:#18E0FF; font-family:Fuwafude,sans-serif;text-shadow:3px 3px 1px #000000,-3px 3px 1px #000000,3px -3px 1px #000000,-3px -3px 1px #000000,3px 0px 1px #000000,0px 3px 1px #000000,-3px 0px 1px #000000,0px -3px 1px #000000">おはさく</span>',
	'hirogaru': 'ひろがる',
	'opa': '<span style="opacity:.25">うすい</span>',
	'bl': '<span style="filter:blur(1.5px) contrast(50%)">ぼやけ</span>',
	'br': '<span style="filter:brightness(200%) drop-shadow(0 0 5px #fff)">輝き</span>',
	'hue': '<span class="hueanim" style="color:red" class="chatcolor">色相アニメ</span>',
	'inv': '<span style="color:black; background-color:white">色相反転</span>',
	'gs': 'モノクロ',
	'vib': '<span class="vib">震え</span>',
	'yoko': '横向き',
	'yax': '<span class="Yaxisrot">Y軸回転</span>',
	'yura': '<span class="yura">ゆらゆら</span>',
};
...
function handleLogin() {
	...
	if (!(CHANNEL.name in obj)) obj[CHANNEL.name] = {
		"last": "",
		data: []
	};
...
function insertbeforeText(str) {
	$chatline.val(str + $chatline.val()).focus();
}

function insertBothText(str1, str2) {
	$chatline.val(str1 + $chatline.val() + str2).focus();
}
...
function playerText(text, classes) {
	...
	if (text !== null && typeof text === "string" && text.length > 0 && !(/^\$/.test(text))) {
		...
		line.addClass(klass).css({
			"visibility": "visible",
			"right": $("#ytapiplayer").width() + "px"
		});
...
function processLayoutElements() {
	...
	var ids = {
		...
		"mainheader": "#chatheader, #videowrap-header",
		"playerlabels": "#videowrap-header .label",
		"chatlabels": "#chatheader .label",
		"playercontrols": "#rightcontrols > div, #db-group",
		"playlistbtns": "#plcontrol > button",
		"playlistmenu": "#plcontrol > div",
		"playermenu": "#videocontrols > div",
		"playerbtns": "#videocontrols > button",
		"mediadbbtns": "#db-group",
	...
	elem.find('a[href$="?oekaki"]').each(function () {
		...
		if (link.indexOf("//i.imgur.com") > -1) {
			...
			var img = $('<img class="oekakiimg" title="Click to open in a new tab" />')
				.attr('src', this.href.replace("?oekaki", ""))
				.load(function () {
					if (SCROLLCHAT) scrollChat();
				});
			$(this).html(img).attr('href', link);
...
function sortFavs(mode) {
	...
	else if (mode == "new") arr.sort(function (a, b) {
...
function mediaTimeLeft(mode) {
	if (mode == "hide") {
		...
		TIMELEFTCLOCK = setInterval(function () {
			timeLeftClock()
		}, 1000);
		timeLeftClock();
	}
...
function titlebarMode(mode) {
	if (mode == "full") {
		...
		MEDIACLOCK = setInterval(function () {
			mediaClock()
		}, 1000);
		mediaClock();
...
if (AnnouncementHTML != "") makeAlert('CTVからのお知らせ☆', AnnouncementHTML).appendTo("#announcements");
...
if (MINIATURES) {
	...
	if (len > 500) setTimeout(function () {
		rebuildMiniatures()
	}, 7500);
	if (len > 1000) setTimeout(function () {
		rebuildMiniatures()
	}, 15000);
	scrollQueue();
}
...
var html = '<button id="commonds-btn" class="btn btn-sm btn-default btn-chatctrl dropdown-toggle" ' +
	'data-toggle="dropdown" aria-expanded="false" title="コマンド"><span class="glyphicon glyphicon-tasks"></span> ▴</button>' +
...
for (i in commonds_arr) {
	...
	var j = commonds_arr.length > 50 ? 8 : 8;
...
$("#my-chat").on("click", function () {
	...
	arr.sort(function (a, b) {
		return (b.last > a.last) ? 1 : ((a.last > b.last) ? -1 : 0)
	});
...
$("#my-messages").on("click", function () {
	...
	function _sort2() {
		...
		for (i in arr) {
			...
			html += '<tr><td>' + nr + '.</td><td>' + text + '</td></tr>';
		}
		$("#tbl-body2").html(html);
	}
	_sort2();
...
$("#layout-1").on("click", function () {
	...
	createModalTabs(['Basic', 'Advanced', 'Show/Hide', 'Filters', 'Media DB', 'HTML', 'CTV独自拡張'], "options");
	...
	var html =
		...
		'			読み上げ' +
		'			<div class="config-col pull-right">' +
		'				<label class="elements-inline">' +
		'					<input id="readchat" type="checkbox" panel="header" mode="2">' +
		'				</label>' +
		'			</div>' +
		'		</div>' +
		'		<div class="panel-body" panel="header">' +
		'			<div class="col-sm-12 config-col">' +
		'				読み上げ音量：<span id="readchatvolumeshow">100%</span>' +
		'				<div id="readchatvolume" class="ui-slider ui-slider-horizontal ui-widget ui-widget-content ui-corner-all" aria-disabled="false">' +
		...
		'			<div class="config-col pull-right">' +
		'				<label class="elements-inline">' +
		'					<input id="micinput" type="checkbox" panel="header" mode="2">' +
		'				</label>' +
		'			</div>' +
		'		</div>' +
		'		<div class="panel-body" panel="header">' +
		'			<div class="col-sm-12 config-col">' +
		'				入力モード' +
		'				<label class="elements-inline" style="margin-left: 20px;font-weight: normal;">' +
		'					<input id="micinput-add" type="radio" panel="header" mode="2" name="micinputmode">' +
		'					追記モード' +
		'				</label>' +
		'				<label class="elements-inline" style="margin-left: 20px;font-weight: normal;">' +
		'					<input type="radio" id="micinput-rewrite" panel="header" mode="2" name="micinputmode">' +
		'					上書きモード' +
		'				</label>' +
		'			</div>' +
		'		</div>' +
		...
		'			<div class="config-col pull-right">' +
		'				<label class="elements-inline">' +
		'					<input id="snowfall" type="checkbox" panel="header" mode="2">' +
		'				</label>' +
		'			</div>' +
		'		</div>' +
		'	</div>' +
	...
	//音量スライダー
	$("#readchatvolume").slider({
		range: "min",
		min: 0,
		max: 100,
		value: READCHATVOLUME * 100,
		change: (function (event, ui) {
			READCHATVOLUME = ui.value / 100;
			$("#readchatvolumeshow").html(Math.floor(READCHATVOLUME * 100) + '%');
			setOpt('SP_readchatvolume', READCHATVOLUME);
		})
	});

	//クッキー☆から設定を読み込む
	if (READCHAT) $("#readchat").prop('checked', true);
	$("#readchatvolumeshow").html(Math.floor(READCHATVOLUME * 100) + '%');
	if (MICINPUT) $("#micinput").prop('checked', true);
	if (MICINPUT_ADDMODE) {
		$("#micinput-add").prop('checked', true);
	} else {
		$("#micinput-rewrite").prop('checked', true);
	}
	if (SNOWFALL) $("#snowfall").prop('checked', true);

	//設定が変わった時クッキー☆に保存
	$("#readchat").on("click", function () {
		setOpt('SP_readchat', READCHAT = $("#readchat").prop('checked'));
	});
	$("#micinput").on("click", function () {
		setOpt('SP_micinput', MICINPUT = $("#micinput").prop('checked'));
	});
	$("#micinput-add").on("click", function () {
		setOpt('SP_micinput_addmode', MICINPUT_ADDMODE = $("#micinput-add").prop('checked'));
	});
	$("#micinput-rewrite").on("click", function () {
		setOpt('SP_micinput_addmode', MICINPUT_ADDMODE = $("#micinput-add").prop('checked'));
	});
	$("#snowfall").on("click", function () {
		setOpt('SP_snowfall', SNOWFALL = $("#snowfall").prop('checked'));
	});
...
$("#plr-14").on("click", function () {
	if ($(this).hasClass('activated')) {
		...
		document.getElementById("plr-11").click();
		if (SINGLECOLUMN || LARGECHAT || LARGEPLAYER) setTimeout(function () {
			$("#plr-13").hide()
		}, 500);
...
$("#sounds-btn").on("click", function () {
	...
	if (!jQuery.isEmptyObject(SoundFiltersArray)) {
		...
		for (i in SoundFiltersArray) {
			...
			$('<td onclick="insertText(\'' + i + '\')">' + i + '</td>').appendTo(tr);
			/*$('<button onclick="insertText(\'' + i + '\')" />')
			  .addClass('btn btn-default').text(i)
			  .appendTo(tr);*/
...
$("#chat-f1").on("click", function () {
	...
	html += '<div class="panel panel-primary"><div class="panel-heading">Random-type commands</div>' +
		...
		'<tr onclick="insertbeforeText(\'' + "!pick " + '\')"><td><code>!pick</code></td><td>choose a random option from a list separated by commas<br />' +
		'(e.g. <i>!pick japan,korea,china</i>)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!ask " + '\')"><td><code>!ask</code></td><td>ask yes-no type question<br />' +
		'(e.g. <i>!ask Am I stupid?</i>)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!fortune " + '\')"><td><code>!fortune</code></td><td>check your daily fate<br />' +
		'<tr onclick="insertbeforeText(\'' + "!emote " + '\')"><td><code>!emote</code></td><td>post random emote</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!dice " + '\')"><td><code>!dice</code></td><td>roll a dice</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!roll " + '\')"><td><code>!roll</code></td><td>roll a 6-digit number</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!game" + '\')"><td><code>!game</code></td><td>simple browser game - guess a number</td></tr>' +
		'</table></div><div class="panel-heading">Info commands</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "!stat " + '\')"><td><code>!stat</code></td><td>display current session user chat statistics</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!time " + '\')"><td><code>!time</code></td><td>display current local time</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!now " + '\')"><td><code>!now</code></td><td>display current media title</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!uagent " + '\')"><td><code>!uagent</code></td><td>display local full user agent string</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!calc " + '\')"><td><code>!calc</code></td><td>do a simple math operation<br />' +
		...
		'<tr onclick="insertbeforeText(\'' + "!add " + '\')"><td><code>!add</code></td><td>add a link to the end of playlist<br />' +
		'(e.g. <i>!add https://www.youtube.com/watch?v=9bZkp7q19f0</i>)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!skip " + '\')"><td><code>!skip</code></td><td>voteskip current item</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!next " + '\')"><td><code>!next</code></td><td>play next item</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!bump " + '\')"><td><code>!bump</code></td><td>bump last item on the playlist to next</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!movernd " + '\')"><td><code>!movernd</code></td><td>move random item to random position ' +
		'on the playlist</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!drop " + '\')"><td><code>!drop</code></td><td>drop first item to the end of playlist</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "!deletelast " + '\')"><td><code>!deletelast</code></td><td>delete last item</td></tr>' +
		'</table></div></div>';
	$("#_c1").html(html);
	...
	var html = 'Type and send to chat text between codes given below, to apply selected effects.<br /><br />' +
		...
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertBothText(\'' + ":+" + '\',\'' + "+:" + '\')"><td><code>:+</code> ...... <code>+:</code></td><td><b>bold</b> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + ":=" + '\',\'' + "=:" + '\')"><td><code>:=</code> ...... <code>=:</code></td><td><i>italic</i> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + ":@" + '\',\'' + "@:" + '\')"><td><code>:@</code> ...... <code>@:</code></td><td><u>underlined</u> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + ":@@" + '\',\'' + "@@:" + '\')"><td><code>:@@</code> ...... <code>@@:</code></td><td>' +
		'<span style="border-bottom:dotted 1px !important">dots-underlined</span> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + ":-" + '\',\'' + "-:" + '\')"><td><code>:-</code> ...... <code>-:</code></td><td><s>striked</s> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + ":!" + '\',\'' + "!:" + '\')"><td><code>:!</code> ...... <code>!:</code></td><td>' +
		...
		'<tr onclick="insertBothText(\'' + ":/" + '\',\'' + "/:" + '\')"><td><code>:/</code> ...... <code>/:</code></td><td>bouncing text ' + ns + '</td></tr>' +
		'<tr onclick="insertBothText(\'' + "://" + '\',\'' + "//:" + '\')"><td><code>://</code> ...... <code>//:</code></td><td>floating text ' + ns + '</td></tr>' +
		'</table></div><div class="panel-heading">BBCodes</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertBothText(\'' + "[b]" + '\',\'' + "[/b]" + '\')"><td><code>[b]</code> ...... <code>[/b]</code></td><td><b>bold</b> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + "[i]" + '\',\'' + "[/i]" + '\')"><td><code>[i]</code> ...... <code>[/i]</code></td><td><i>italic</i> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + "[u]" + '\',\'' + "[/u]" + '\')"><td><code>[u]</code> ...... <code>[/u]</code></td><td><u>underlined</u> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + "[s]" + '\',\'' + "[/s]" + '\')"><td><code>[s]</code> ...... <code>[/s]</code></td><td><s>striked</s> text</td></tr>' +
		'<tr onclick="insertBothText(\'' + "[code]" + '\',\'' + "[/code]" + '\')"><td><code>[code]</code> ...... <code>[/code]</code></td>' +
		'<td><code>code-like</code> text</td></tr>' +
		'</table></div><div class="panel-heading">CTV☆Codes</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "[d]" + '\')"><td><code>[d]</code></td><td><span class="dist">dist</span> text</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[round0]" + '\')"><td><code>[round0]</code> </td><td>遅往復</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[round]" + '\')"><td><code>[round]</code> </td><td>往復</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[round1]" + '\')"><td><code>[round1]</code> </td><td>早往復</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[rscroll]" + '\')"><td><code>[rscroll]</code> </td><td>通過</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[rscroll200]" + '\')"><td><code>[rscroll200]</code> </td><td>超通過</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[usagi]" + '\')"><td><code>[usagi]</code> </td><td><span class="pyonpyon">うさぎ飛び</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[yu]" + '\')"><td><code>[yu]</code> </td><td><span style="font-size:16px;color:#FFFFFF; font-family:HG創英角ﾎﾟｯﾌﾟ体,sans-serif;text-shadow:0px 0px 4px #000000,0px 0px 4px #000000,0px 0px 4px #000000">じゃあな！</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[an]" + '\')"><td><code>[an]</code> </td><td><span style="font-size:20px;color:#18E0FF; font-family:Fuwafude,sans-serif;text-shadow:3px 3px 1px #000000,-3px 3px 1px #000000,3px -3px 1px #000000,-3px -3px 1px #000000,3px 0px 1px #000000,0px 3px 1px #000000,-3px 0px 1px #000000,0px -3px 1px #000000">おはさく</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[hirogaru]" + '\')"><td><code>[hirogaru]</code> </td><td><span style="letter-spacing: 30pt">ひろがる</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[opa]" + '\')"><td><code>[opa]</code> </td><td><span style="opacity:.25"> うすい </span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[bl]" + '\')"><td><code>[bl]</code> </td><td><span style="filter:blur(1.5px) contrast(50%)"> ぼやけ </span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[br]" + '\')"><td><code>[br]</code> </td><td><span style="filter:brightness(200%) drop-shadow(0 0 5px #fff)"> 輝き </span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[hue]" + '\')"><td><code>[hue]</code> </td><td><span class="hueanim" style="color:red" class="chatcolor">色相アニメーション </span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[inv]" + '\')"><td><code>[inv]</code> </td><td><span style="color:black; background-color:white">色相反転</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[gs]" + '\')"><td><code>[gs]</code> </td><td>モノクロ</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[yoko]" + '\')"><td><code>[yoko]</code> </td><td><span class="m_mark_yoko">横向き</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[yax]" + '\')"><td><code>[yax]</code> </td><td><span class="Yaxisrot">Y軸回転</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "@" + '\')"><td><code>@</code> </td><td><span style="display:inline-block;transform:rotateY(180deg)">かがみ</span>(エモート)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[yura]" + '\')"><td><code>[yura]</code> </td><td><span class="yura">ゆらゆら</span></td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "[vib]" + '\')"><td><code>[vib]</code> </td><td><span class="vib">なぜか高鳴る胸の鼓動を抑える運動ー</span></td></tr>'
	...
	var html = '<div class="panel panel-primary">' +
		...
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "/help" + '\')"><td><code>/help</code></td><td>open this modal window</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/premium" + '\')"><td><code>/premium</code></td><td>open "Premium Settings" modal window</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/css" + '\')"><td><code>/css</code></td><td>open "Theme & User CSS" modal window</td></tr>' +
		'</table></div><div class="panel-heading">General layout commands</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "/theatre" + '\')"><td><code>/theatre</code></td><td>switch to Theatre Mode</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/radio" + '\')"><td><code>/radio</code></td><td>switch to Radio Mode</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/close" + '\')"><td><code>/close</code></td><td>close Theatre or Radio Mode</td></tr>' +
		'</table></div><div class="panel-heading">Chat commands</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "/leader" + '\')"><td><code>/leader</code></td><td>' +
		'toggle your Leader status (if you have permission)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/chatclear" + '\')"><td><code>/chatclear</code></td><td>clear current content of your chat window</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/emotes" + '\')"><td><code>/emotes</code></td><td>open handy "Emotes" panel</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/unicode" + '\')"><td><code>/unicode</code></td><td>open "Unicode characters" panel</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/msg" + '\')"><td><code>/msg</code></td><td>open "My Messages" modal window</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/expchat" + '\')"><td><code>/expchat</code></td><td>toggle chat expanding</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/autoscroll" + '\')"><td><code>/autoscroll</code></td><td>toggle chat autoscroll</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/ulist" + '\')"><td><code>/ulist</code></td><td>show/hide userlist</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/ulistside" + '\')"><td><code>/ulistside</code></td><td>toggle userlist side</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/top" + '\')"><td><code>/top</code></td><td>scroll chat panel to top<td></td></tr>' +
		'</table></div><div class="panel-heading">Player commands</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "/plhide" + '\')"><td><code>/plhide</code></td><td>toggle "Hide Player Until Next" function</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/pltoggle" + '\')"><td><code>/pltoggle</code></td><td>toggle player displaying</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/plmute" + '\')"><td><code>/plmute</code></td><td>toggle player muting</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/nn" + '\')"><td><code>/nn</code></td><td>toggle NicoNico mode ' +
		'(chat messages displayed on player)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/mascot" + '\')"><td><code>/mascot</code></td><td>toggle player mascot</td></tr>' +
		'</table></div><div class="panel-heading">Playlist commands</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "/last" + '\')"><td><code>/last</code></td><td>open "Last Played" modal window</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/save" + '\')"><td><code>/save</code></td><td>download current item (if available)</td></tr>' +
		'<tr onclick="insertbeforeText(\'' + "/fav" + '\')"><td><code>/fav</code></td><td>add current item to your Premium favourites</td></tr>' +
		'</table></div><div class="panel-heading">Account commands</div>' +
		'<div class="panel-body"><table class="commands-tbl">' +
		'<tr onclick="insertbeforeText(\'' + "/hist" + '\')"><td><code>/hist</code></td><td>open "My Chat History" modal window</td></tr>' +
	...
	for (var i = 0; i < 10; i++) {
		var html = '<td onclick="insertText(\'' + "//" + i + '\')"><code>//' + i + '</code></td><td>' +
	...
	var rows = [
		['1', 'Theatre Mode'],
		['2', 'Radio Mode'],
		['3', '"Premium Settings" modal window'],
		['4', '"Theme & User CSS" modal window'],
		['5', '"Large Chat, No Player" option'],
		['6', '"Large Player, No Chat" option'],
		['7', 'Toggle chat expanding'],
		['8', 'Hide/show playlist'],
		['9', 'Mute player'],
		['0', 'Close Theatre/Radio Mode'],
		['Q', 'Volume level up'],
		['A', 'Volume level down'],
		['T', 'Scroll chat panel to top'],
		['N', 'Toggle chat messages on player (NicoNico style)'],
		['I', 'Toggle converting chat links to images'],
		['F', 'Add current item to your Premium favourites'],
	...
	if (!$(this).hasClass('activated')) {
		...
		var tbl = [{
			...
			},
			{
				...
				't1': ':^',
				't2': '^:',
				's': 'outline:1px dashed #98ABB9; outline-offset:-5px; ' +
			...
			$('<button class="btn btn-primary btn-sm">Install chat text colors</button>')
				...
				.on("click", function () {
					socket.emit("addFilter", {
						name: 'chat colors (premium)',
						source: 'col:(.*?):',
						flags: 'g',
						replace: '<span style="color:\\1" class="chatcolor">',
						active: true
					});
		...
		if (count < 1) {
			...
			$('<button class="btn btn-primary btn-sm">Install chat text effects</button>')
				...
				.on("click", function () {
					...
					socket.emit("addFilter", {
						name: 'dots-underlined text (premium)',
						source: ':@@(.+?)@@:',
						flags: 'g',
					...
					setTimeout(function () {
						...
						socket.emit("addFilter", {
							name: 'letter-spaced text (premium)',
							source: ':%(.+?)%:',
							flags: 'g',
					...
					setTimeout(function () {
						...
						socket.emit("addFilter", {
							name: 'italic text (premium BBCode)',
							source: '\\[i\\](.+?)\\[\\/i\\]',
							flags: 'g',
							replace: '<span style="font-style:italic" class="txteffect">\\1</span>',
							active: true
						...
						socket.emit("addFilter", {
							name: 'striked text (premium BBCode)',
							source: '\\[s\\](.+?)\\[\\/s\\]',
							flags: 'g',
							replace: '<span class="txteffect"><s>\\1</s></span>',
							active: true
...
$("#antiafk-btn").on("click", function () {
	if ($(this).hasClass('btn-warning')) clearInterval(ANTIAFK)
		...
		ANTIAFK = setInterval(function () {
			if (findUserlistItem(CLIENT.name).data().meta.afk) socket.emit("chatMsg", {
				msg: '/afk'
			});
		}, 2000);
	}
...
$scrollToCurrent.on("click", function () {
	...
	setTimeout(function () {
		$scrollToCurrent.removeClass('label-success')
	}, 1000);
});
...
CyTube.tabCompleteMethods['Longest unique match'] = function (input, position, options, context) {
	...
	if (matches.length === 0) return {
		...
		for (var i = 0; i < matches.length - 1; i++) {
			...
			var nextPrefix = '';
			for (var j = 0;
				(currentPrefix === null || j < currentPrefix.length) &&
...
function formatChatMessage(data, last) {
	...
	if (!MUTECHAT && (!(data.username in MUTEDVOICES) || MUTEDVOICES[data.username] == "0")) {
		for (i in SoundFiltersArray) {
			if (data.msg.indexOf(i) > -1) {
				...
				setTimeout(function () {
					$("#sounds-btn").removeClass('btn-success')
				}, 1000);
			}
		}
	}
	if (TEXTTOSPEECH) {
		...
		if (TTSTHROTTLE < 3) {
			...
			str = str.replace(/(!time|!now|!stat|!uagent|!calc|!game|!add|!skip)/g, "");
			str = str.replace(/(!next|!bump|!movernd|!drop|!deletelast)/g, "");
			//CTV独自拡張コマンド向け箇所
			str = str.replace(/(!kore)/g, "");
	...
	if (msg.indexOf('!') == 0) {
		...
		if (msg.indexOf('!pick ') == 0) {
			...
			if (COMMAND) {
				...
				msg = CYTUBE + FortunesArray[Math.round(Math.random() * (FortunesArray.length - 1))];
				FORTUNE = true;
			}
			...
			if (hasPermission("playlistadd")) {
				...
				if (parsed["id"] === null) addChatNotification('Error: invalid link')
					...
					addChatNotification('動画が追加されました☆');
				}
			...
			if (hasPermission("voteskip")) {
				...
				if ($queue.find(".queue_entry").length > 0) addChatNotification('スキップ投票を行いました☆')
			...
			//ここ以下CTV独自拡張コマンドの処理
		} else if (msg.indexOf('!kore') == 0) {
			msg = korearray[Math.floor(korearray.length*Math.random())]; 
		} else COMMAND = false;
	}
	return msg;
}
...
$(document).on('keydown', function (e) {
	if (e.ctrlKey && !e.altKey) {
		if (String.fromCharCode(e.which) === "Z") document.getElementsByClassName("drawing-board-control-navigation-back")[0].click()
		else if (String.fromCharCode(e.which) === "Y") document.getElementsByClassName("drawing-board-control-navigation-forward")[0].click()
	}
});
...
function uploadOekaki(pic) {
	...
	$.ajax({
		...
		data: {
			image: pic.replace(/^data:image\/[a-z]+;base64,/, ''),
			type: 'base64'
		},
		dataType: 'json',
		success: function (json) {
			//socket.emit("chatMsg", {msg:json.data.link + '?oekaki'});
			insertText(json.data.link + '?oekaki');
			upalert.remove();
		...
		DrawingBoard.Control.Upload = DrawingBoard.Control.extend({
			...
			initialize: function () {
				...
				this.$el.on('click', '#oeup-btn', $.proxy(function (e) {
					e.preventDefault();
					var time = (new Date()).getTime();
					if ((time - OETSTAMP) > 10000) {
						OETSTAMP = time;
						uploadOekaki(this.board.getImg());
					} else alert('Warning! You can send 1 picture every 10 seconds.');
				}, this));
			}
		});
...
$oekakiwrap = $('<div id="oekakiwrap" class="col-lg-12 col-md-12 wells leftareas" />')
	.insertBefore("#pollwrap").append('<div id="oekaki-well" class="well form-horizontal" />').hide();
...
var css = '.bigtitle {\n' +
	...
	'.btn-commonds {margin-left:0px !important}\n' +
	...
	'#radioslider a, #chatslider a, #readchatvolume a {background:none; background-color:#F00; cursor:pointer}\n';
...
if (CHATTOTOP) setTimeout(function () {
	scrollChatToTop()
}, 500);
...
//今の所YouTube,niconico,dailymotion,Twitchの動画の時にのみ対応
(function () {
	//
	//要素の追加
	//
	...
	tweetbutton.id = "tweet-btn";
	tweetbutton.classList.add("btn", "btn-sm", "btn-default", "btn-chatctrl");
	tweetbutton.innerHTML = "Tw"; //ボタンに表示するもの(仮)
	chatControls.insertBefore(tweetbutton, chatControls.firstElementChild.nextElementSibling);

	//表示領域の要素を作る
	var leftPaneInner = document.getElementById("leftpane-inner");
	var tweetWrap = document.createElement("div");
	tweetWrap.id = "tweetwrap";
	tweetWrap.classList.add("col-lg-12", "col-md-12", "wells", "leftareas");
	tweetWrap.innerHTML =
		'<div id="tweet-well" class="well form-horizontal">' +
		'	<div style="text-align:left">' +
		'		<label for="isMediaLengthAdded">' +
		'			<input id="isMediaLengthAdded" type="checkbox" class="cs-checkbox">' +
		'			動画の再生時間を含める' +
		'		</label><br>' +
		'		<label for="isCurrentPositionAdded">' +
		'			<input id="isCurrentPositionAdded" type="checkbox" class="cs-checkbox">' +
		'			現在の動画位置を含める' +
		'		</label><br>' +
		'	</div>' +
		'		<textarea readonly rows="7" id="tweetText" class="form-control" style="text-align: left;"></textarea><br>' +
		'	<div style="text-align:center">' +
		'		ツイートが声優や企画者に見つかると転載動画の削除や迷惑に繋がります。<br>' +
		'		この機能は節度をわきまえて使用しましょう。<br>' +
		'		<a id="junpToComposeTweet" target="_blank">' +
		'			<button class="btn btn-sm btn-default">ツイートへ</button>' +
		'		</a>' +
		'	</div>' +
		'</div>';

	tweetWrap.style.display = "none"; //非表示にする
	leftPaneInner.insertBefore(tweetWrap, leftPaneInner.firstElementChild.nextElementSibling);

	//
	//ツイートのために情報を取得する関数、オブジェクトたち
	//
	...
	function setPlayer() {
		if (PLAYER.yt) {
			...
			kindOfPlayer = "TwitchTV";
		} else {
			//対応していなければ中止し、フォーム等非表示に
			alert("この動画にツイート機能は未対応です")
			$(tweetbutton).toggleClass("btn-success");
			tweetWrap.style.display = "none";
		}
	...
	var baseURLs = {
		...
		TwitchTV: "https://www.twitch.tv/videos/"
	};
	...
	function getCurrentPosition() {
		...
		case "Dailymotion":
			...
			break;
		case "TwitchTV":
			output = PLAYER.twitch.getCurrentTime();
			break;
		}
		return Math.round(output);
	}

	//秒数の値をhh:mm:ssにフォーマット　グローバルの同名関数を隠蔽
	function timeFormat(second) {
		var outputSeconds = second % 60;
		var outputMinutes = (second - outputSeconds) / 60 % 60;
		var outputHours = (second - outputSeconds - 60 * outputMinutes) / 3600;
		outputSeconds = outputSeconds < 10 ? "0" + outputSeconds : "" + outputSeconds;
		outputMinutes = outputMinutes < 10 ? "0" + outputMinutes : "" + outputMinutes;
		outputHours = outputHours < 10 ? "0" + outputHours : "" + outputHours;
	...
	var mediaLength;
	var currentPosition;
	var adminTweetString = window.eventTweetString||"@CookieTV_senden より"; //管理者が設定する、ツイートに加えられる文字列　ハッシュタグはhashtagsに入れてください
	var hashtags = window.eventHashtags||"クッキーTV"; //複数登録の際はカンマ区切りで
	...
	//よく使うDOM要素
	var mediaLengthCheck = document.getElementById("isMediaLengthAdded");
	var currentPositionCheck = document.getElementById("isCurrentPositionAdded");

	//チャット欄下のボタンがクリックされた時、ツイート投稿のための要素を表示してデータをセット
	function onTwButtonClicked() {
		$(tweetbutton).toggleClass("btn-success"); //JQuery関数
		setPlayer();
		if (tweetbutton.classList.contains("btn-success")) {
			tweetWrap.style.display = "block";
			playingTitle = document.getElementById("currenttitle").innerHTML.replace("&amp;", "&")
			mediaLength = timeFormat(PLAYER.mediaLength);
			currentPosition = timeFormat(getCurrentPosition());
			onCheckBoxesChanged();
		} else {
			tweetWrap.style.display = "none";
		}
	...
	//チェックボックスの内容が変更された時、ツイート文字列を変更し、テキストエリアと「ツイートへ」ボタンにセット
	function onCheckBoxesChanged() {
		var tweetTextShow = ""; //テキストエリアに入れて見せるやつ
		var tweetMainContent = ""; //URLの一部になるやつ
		var lf = encodeURIComponent("\n"); //エンコード済みの改行記号

		tweetTextShow += playingTitle;
		tweetMainContent += encodeURIComponent(playingTitle);

		if (mediaLengthCheck.checked) {
			if (currentPositionCheck.checked) {
				tweetTextShow += "\n(" + currentPosition + "地点/" + mediaLength + ")";
				tweetMainContent += lf + "(" + currentPosition + "地点/" + mediaLength + ")";
			} else {
				tweetTextShow += "\n(" + mediaLength + ")";
				tweetMainContent += lf + "(" + mediaLength + ")";
			}
		} else if (currentPositionCheck.checked) {
			tweetTextShow += "\n(" + currentPosition + "地点)";
			tweetMainContent += lf + "(" + currentPosition + "地点)";
		}
		tweetTextShow += "\n" + adminTweetString;
		tweetMainContent += lf + encodeURIComponent(adminTweetString);

		var hashtagsArray = hashtags.split(",");
		for (var i = 0; i < hashtagsArray.length; i++) {
			tweetTextShow += " #" + hashtagsArray[i];
		}
		document.getElementById("tweetText").textContent = tweetTextShow;
		document.getElementById("junpToComposeTweet").href =
			"https://twitter.com/intent/tweet?text=" + tweetMainContent + "&hashtags=" + hashtags;
	}

	//イベントハンドラをセット
	tweetbutton.onclick = onTwButtonClicked;
	mediaLengthCheck.onclick = onCheckBoxesChanged;
	currentPositionCheck.onclick = onCheckBoxesChanged;

})();

//チャット読み上げ機能
(function () {
	//korehaTaihenさんに頂いた、読み上げを止めないためのコードを少し改変したもの
	var pauseResume;
	var speakChatMessage = function(synthes) {
		if(speechSynthesis.speaking == false) pauseResume = setInterval(speakPauseResume, 10000);
		
		speechSynthesis.speak(synthes);
	};

	var speakPauseResume= function(){
		if(speechSynthesis.speaking == false) clearInterval(pauseResume);
		speechSynthesis.pause();
		speechSynthesis.resume();
	};
	
	//ここまでは発声しましたっていうDOM
	var $lastspoke = $messagebuffer[0].lastElementChild;

	//100ミリ秒ごとにメッセージがあるか確認し、あれば一つ読む
	setInterval(function () {
		if ($lastspoke.nextElementSibling) {
			$lastspoke = $lastspoke.nextElementSibling;

			//発声しないとき
			if (!READCHAT) return;

			var synthes = new SpeechSynthesisUtterance();
			synthes.volume = READCHATVOLUME;
			synthes.lang = 'ja-JP';
			//urlを置換する
			var reg=new RegExp("((https?|ftp)(:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+))");
			synthes.text = $lastspoke.lastElementChild.innerText.replace(reg,"URL省略");
			speakChatMessage(synthes);
		}
	}, 100);
})();
...
(function () {
	...
	//初期設定
	if (!MICINPUT) return; //ロード時にオンオフ設定を見て適用
	if (!window.webkitSpeechRecognition) return; //ライブラリがない場合
	...
	rec.onresult = e => {
		//録音ストップ
		rec.stop();
		var mestosend = e.results[0][0]["transcript"];
		//エンターっていう発言だったらメッセージを送る
		if (/^えんたー$|^エンター$/.test(mestosend)) {
			sendMessage();
			return;
		} else if (/^くりあ$|^クリア$/.test(mestosend)) { //クリアっていう発言だったらチャット欄クリア
			$chatline[0].value = "";
			return;
		}
		...
		for (var i = 0; i < replacements.length; i++) {
			mestosend = mestosend.replace(replacements[i].reg, replacements[i].repwith);
		}

		if (MICINPUT_ADDMODE) { //追記モードなら
			$chatline[0].value += mestosend;
		} else { //上書きモードなら
			$chatline[0].value = mestosend;
		}
...
(function () {
	if(!SNOWFALL) return;
	$.getScript("https://cytube.xyz/up/src/up22349.js", function(){
		$(document).snowfall(
			{
				flakeCount : 35, // 雪の量 (数値)
				flakeColor : "#ffffff", // 色 (RGB)
				flakeIndex : "999", // z-indexの値
				minSize : 1, // 最小サイズ （数値）
				maxSize : 2, // 最大サイズ（数値）
				minSpeed : 1, // 最低速度（数値）
				maxSpeed : 5, // 最高速度（数値）
				round : true, // 雪の形を丸にする（boolean）
				shadow : false, // 影をつける（boolean）
				//collection : "span", // 要素に積もらせる場合
				//image : "画像へのパス" // オリジナル画像を使用する場合
			}
		);
	});
})();
...
(function () {
	//検索周りの変数を初期化
	EMOTELIST.initSearch();
	//フィルター
	EMOTELIST.filter = function(emote){
		var value=document.getElementById("emotelist").getElementsByClassName("emotelist-search")[0].value.toLowerCase().split(" ");
		var output=true;
		for(var i=0;i<value.length;i++){
			if(emote.name.toLowerCase().indexOf(value[i])<0)output=false;
		}
		return output;
	};
	//古いイベントリスナを削除
	var sb=EMOTELIST.searchbar[0];
	var newsb=sb.cloneNode(true);
	sb.parentNode.replaceChild(newsb,sb);
	//新しいハンドラをセット
	newsb.onkeyup=function(){
